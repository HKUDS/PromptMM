...
class Teacher_Model(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, weight_size, dropout_list, image_feats, text_feats, audio_feats):
    ...
    # def forward(self, ui_graph, iu_graph):
        ...
        feat_prompt_item_image = torch.mean( torch.stack((torch.mm(prompt_item, torch.mm(prompt_item.T, self.image_feats)), torch.mm(prompt_item, torch.mm(prompt_item.T, self.image_feats))), dim=0) , dim=0 )
        feat_prompt_item_text = torch.mean( torch.stack((torch.mm(prompt_item, torch.mm(prompt_item.T, self.text_feats)), torch.mm(prompt_item, torch.mm(prompt_item.T, self.text_feats))), dim=0) , dim=0 )
        feat_prompt_item_audio = torch.mean( torch.stack((torch.mm(prompt_item, torch.mm(prompt_item.T, self.audio_feats)), torch.mm(prompt_item, torch.mm(prompt_item.T, self.audio_feats))), dim=0) , dim=0 )
        ...
        audio_feats = audio_item_feats = self.dropout(self.audio_trans(self.audio_feats + args.feat_soft_token_rate*F.normalize(feat_prompt_item_audio, p=2, dim=1) ))  
        ...
        for i in range(args.layers):
            ...
            audio_user_feats = self.mm(ui_graph, audio_feats)
            audio_item_feats = self.mm(iu_graph, audio_user_feats)
        ...
        u_g_embeddings = u_g_embeddings + args.model_cat_rate*F.normalize(image_user_feats, p=2, dim=1) + args.model_cat_rate*F.normalize(text_user_feats, p=2, dim=1) + args.model_cat_rate*F.normalize(audio_user_feats, p=2, dim=1)
        i_g_embeddings = i_g_embeddings + args.model_cat_rate*F.normalize(image_item_feats, p=2, dim=1) + args.model_cat_rate*F.normalize(text_item_feats, p=2, dim=1) + args.model_cat_rate*F.normalize(audio_item_feats, p=2, dim=1)

        return u_g_embeddings, i_g_embeddings, image_item_feats, text_item_feats, audio_item_feats, image_user_feats, text_user_feats, audio_user_feats, u_g_embeddings, i_g_embeddings  , prompt_user, prompt_item
...
class PromptLearner(nn.Module):
    def __init__(self, image_feats=None, text_feats=None, audio_feats=None, ui_graph=None):
        super().__init__()
        ...
        if args.hard_token_type=='pca':
            try:
                ...
                hard_token_audio = pickle.load(open(args.data_path + args.dataset + '/hard_token_audio_pca','rb'))
                ...
                hard_token_audio = PCA(n_components=args.embed_size).fit_transform(audio_feats)
                ...
                pickle.dump(hard_token_audio, open(args.data_path + args.dataset + '/hard_token_audio_pca','wb'))
            ...
            try:
                ...
                hard_token_audio = pickle.load(open(args.data_path + args.dataset + '/hard_token_audio_ica','rb'))
                print('already load hard token', time() - t1)
            except Exception:
                ...
                hard_token_audio = FastICA(n_components=args.embed_size, random_state=12).fit_transform(audio_feats)
                ...
                pickle.dump(hard_token_audio, open(args.data_path + args.dataset + '/hard_token_audio_ica','wb'))
            ...
            hard_token_audio = manifold.Isomap(n_neighbors=5, n_components=args.embed_size, n_jobs=-1).fit_transform(audio_feats)
        ...
        self.item_hard_token = torch.mean((torch.stack((torch.tensor(hard_token_image).float(), torch.tensor(hard_token_text).float(), torch.tensor(hard_token_audio).float()))), dim=0).cuda()
...
class Student_LightGCN(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, gnn_layer, dropout_list, image_feats=None, text_feats=None, audio_feats=None):
        super().__init__()
        self.n_users = n_users
        self.n_items = n_items
        self.embedding_dim = embedding_dim
...
class Student_GCN(nn.Module):
    def __init__(self, n_users, n_items, embedding_dim, gnn_layer=2, drop_out=0., image_feats=None, text_feats=None, audio_feats=None):
