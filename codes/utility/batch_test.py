...
def test_one_user(x):
    ...
    if args.test_flag == 'part':
        r, auc = ranklist_by_heapq(user_pos_test, test_items, rating, eval(args.Ks))
    else:
        r, auc = ranklist_by_sorted(user_pos_test, test_items, rating, eval(args.Ks))

    return get_performance(user_pos_test, r, auc, eval(args.Ks))
...
def test_torch(ua_embeddings, ia_embeddings, users_to_test, is_val, drop_flag=False, batch_test_flag=False):
    result = {'precision': np.zeros(len(eval(args.Ks))), 'recall': np.zeros(len(eval(args.Ks))), 'ndcg': np.zeros(len(eval(args.Ks))),
              'hit_ratio': np.zeros(len(eval(args.Ks))), 'auc': 0.}
