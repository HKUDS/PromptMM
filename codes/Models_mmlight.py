...
class Student_MMLight(nn.Module):
    ...
    def forward(self, image_feats, text_feats, image_user_embeds, text_user_embeds):
        ...
        # image_user_feats = self.mm(self.ui_graph, image_feats)
        # image_item_feats = self.mm(self.iu_graph, image_user_feats)
        # text_user_feats = self.mm(self.ui_graph, text_feats)
        ...
        # return u_g_embeddings, i_g_embeddings, image_item_feats, text_item_feats, image_user_feats, text_user_feats, u_g_embeddings, i_g_embeddings
...
class Student_LightGCN(nn.Module):
    ...
    def forward(self, adj, image_item_embeds, text_item_embeds, image_user_embeds, text_user_embeds):
        ...
        u_g_embeddings = u_g_embeddings + args.model_cat_rate*F.normalize(image_user_embeds, p=2, dim=1) + args.model_cat_rate*F.normalize(text_user_embeds, p=2, dim=1)
        i_g_embeddings = i_g_embeddings + args.model_cat_rate*F.normalize(image_item_embeds, p=2, dim=1) + args.model_cat_rate*F.normalize(text_item_embeds, p=2, dim=1)
