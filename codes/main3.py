...
from Models3 import Teacher_Model, Student_LightGCN, Student_GCN, Student_MLP, PromptLearner
...
class Trainer(object):
    def __init__(self, data_config):
        ...
        self.audio_feat_dim = self.audio_feats.shape[-1]
        ...
        self.image_ui_graph = self.text_ui_graph = self.audio_ui_graph = self.ui_graph
        self.image_iu_graph = self.text_iu_graph = self.audio_iu_graph = self.iu_graph

        self.teacher_model = Teacher_Model(self.n_users, self.n_items, self.emb_dim, self.weight_size, self.mess_dropout, self.image_feats, self.text_feats, self.audio_feats)      
        ...
        self.prompt_module = PromptLearner(self.image_feats, self.text_feats, self.audio_feats, self.ui_graph)
    ...
    def feat_reg_loss_calculation(self, g_item_image, g_item_text, g_item_audio, g_user_image, g_user_text, g_user_audio):
        feat_reg = 1./2*(g_item_image**2).sum() + 1./2*(g_item_text**2).sum() + 1./2*(g_item_audio**2).sum()\
            + 1./2*(g_user_image**2).sum() + 1./2*(g_user_text**2).sum() + 1./2*(g_user_audio**2).sum()        
    ...
    def train(self):
        ...
        if args.if_train_teacher: 
            ...
            for epoch in range(args.epoch):
                ...
                for idx in tqdm(range(n_batch)):
                    ...
                    # self.prompt_module()
                    t_u_id_embed, t_i_id_embed, t_i_image_embed, t_i_text_embed, t_i_audio_embed, t_u_image_embed, t_u_text_embed, t_u_audio_embed \
                    ...
                    t_audio_u_g_embeddings = t_u_audio_embed[users]
                    t_audio_pos_i_g_embeddings = t_i_audio_embed[pos_items]
                    t_audio_neg_i_g_embeddings = t_i_audio_embed[neg_items]
                    t_audio_batch_mf_loss, G_audio_batch_emb_loss = self.bpr_loss(t_audio_u_g_embeddings, t_audio_pos_i_g_embeddings, t_audio_neg_i_g_embeddings)

                    feat_emb_loss = self.feat_reg_loss_calculation(t_i_image_embed, t_i_text_embed, t_i_audio_embed, t_u_image_embed, t_u_text_embed, t_u_audio_embed)
                    ...
                    t_batch_loss = t_mf_loss + t_emb_loss + feat_emb_loss + args.t_prompt_rate1*mf_loss_prompt + args.t_feat_mf_rate*t_image_batch_mf_loss + args.t_feat_mf_rate*t_text_batch_mf_loss + args.t_feat_mf_rate*t_audio_batch_mf_loss
                ...
                del t_u_id_embed, t_i_id_embed, t_i_image_embed, t_i_text_embed, t_i_audio_embed, t_u_image_embed, t_u_text_embed, t_u_audio_embed \
        ...
        with torch.no_grad():
            self.u_final_embed, self.i_final_embed, image_item_embeds, text_item_embeds, audio_item_embeds, image_user_embeds, text_user_embeds, audio_user_embeds \
            , G_user_emb, G_item_emb, prompt_user, prompt_item \
            = self.teacher_model(self.ui_graph, self.iu_graph, self.prompt_module)

        if args.student_model_type=='lightgcn':
            self.student_model = Student_LightGCN(self.n_users, self.n_items, self.student_emb_dim, self.student_n_layers, self.mess_dropout, self.image_feats, self.text_feats, self.audio_feats)   
            self.student_model.init_user_item_embed(self.u_final_embed, self.i_final_embed)
        elif args.student_model_type=='gcn': 
            self.student_model = Student_GCN(self.n_users, self.n_items, self.student_emb_dim, self.student_n_layers, self.mess_dropout, self.image_feats, self.text_feats, self.audio_feats)   
        elif args.student_model_type=='mlp': 
        ...
        for epoch in range(args.epoch):
            ...
            for idx in tqdm(range(n_batch)):
                ...
                # list_wise_loss = -list_wise_score[:,0].log().sum()
                ...
                list_wise_score_t_audio = torch.mul(audio_user_embeds[users].unsqueeze(1), audio_item_embeds[item_index]).sum(-1).softmax(-1) 
                # list_wise_loss_t_audio = -(list_wise_score_t_audio[:,0]+1e-8).log()
                list_wise_loss_t_audio = -(list_wise_score_t_audio+1e-8).log()
                ...
                audio_u_embed = audio_user_embeds[users]
                audio_pos_i_embed = audio_item_embeds[pos_items]
                audio_neg_i_embed = audio_item_embeds[neg_items]
                audio_teacher_mf_loss, G_audio_batch_emb_loss, G_audio_batch_reg_loss = self.bpr_loss_for_KD(audio_u_embed, audio_pos_i_embed, audio_neg_i_embed)
                ...
                audio_kd_loss = self.distillation(student_mf_loss, audio_teacher_mf_loss, temp=args.student_tau, alpha=0.7)
                ...
                kd_loss_list_audio = self.distillation(list_wise_loss_s, list_wise_loss_t_audio, temp=args.student_tau, alpha=0.7)
                ...
                if args.feat_loss_type=='mse':
                    ...
                    kd_loss_feat = self.sce_criterion(self.i_final_embed, i_embed, alpha=args.alpha_l, tip_rate=args.tip_rate_feat)
                # ----feat kd loss-----------------------------------------------------------------------------------------------------------
                ...
                # student_batch_loss = batch_mf_loss + batch_emb_loss + batch_reg_loss + args.kd_loss_rate*kd_loss + args.kd_loss_image_rate*image_kd_loss + args.kd_loss_text_rate*text_kd_loss #+ G_batch_reg_loss + feat_emb_loss
                # student_batch_loss = batch_mf_loss + batch_emb_loss + batch_reg_loss + args.kd_loss_rate*kd_loss + args.kd_loss_text_rate*text_kd_loss #+ G_batch_reg_loss + feat_emb_loss

                # student_batch_loss = batch_mf_loss + batch_emb_loss + batch_reg_loss + args.list_wise_loss_rate*list_wise_loss # + args.kd_loss_rate*kd_loss + args.kd_loss_image_rate*image_kd_loss + args.kd_loss_text_rate*text_kd_loss #+ G_batch_reg_loss + feat_emb_loss
                # student_batch_loss = batch_mf_loss + batch_emb_loss + batch_reg_loss + args.kd_loss_rate*kd_loss #+ args.kd_loss_image_rate*image_kd_loss + args.kd_loss_text_rate*text_kd_loss #+ G_batch_reg_loss + feat_emb_loss

                # student_batch_loss = batch_mf_loss + batch_emb_loss + args.student_reg_rate*batch_reg_loss + args.list_wise_loss_rate*list_wise_loss + args.kd_loss_rate*kd_loss + args.kd_loss_image_rate*image_kd_loss +  args.kd_loss_text_rate*text_kd_loss + kd_loss_list + kd_loss_list_text #+ kd_loss_list_image #+ kd_loss_list_text
                # student_batch_loss = batch_mf_loss + batch_emb_loss + args.kd_loss_rate*kd_loss #+ args.kd_loss_image_rate*image_kd_loss +  args.kd_loss_text_rate*text_kd_loss #+ args.kd_loss_list_rate*kd_loss_list + reg_loss #+ kd_loss_list_image + kd_loss_list_text
                # student_batch_loss = batch_mf_loss + batch_emb_loss + args.kd_loss_rate*kd_loss + args.kd_loss_list_rate*kd_loss_list + args.kd_loss_feat_rate*kd_loss_feat    #
                student_batch_loss = batch_mf_loss + batch_emb_loss + args.kd_loss_rate*kd_loss + args.kd_loss_list_rate*kd_loss_list + args.kd_loss_feat_rate*kd_loss_feat + args.kd_loss_rate*image_kd_loss + args.kd_loss_rate*text_kd_loss + args.kd_loss_rate*audio_kd_loss + args.kd_loss_list_rate*kd_loss_list_image + args.kd_loss_list_rate*kd_loss_list_text + args.kd_loss_list_rate*kd_loss_list_audio
